...
@section('title', 'Editar Grado Escolar')
  ...
  <div class="col-md-8 col-sm-12 mt-2 pb-3 rounded shadow bg-white border">
    ...
    <div class="d-flex align-items-center justify-content-between p-2 my-2 rounded shadow-sm border">
      <h5 class="mb-0 lh-100 text-uppercase">
          <i class="fas fa-pencil-alt text-info"></i> editar grado escolar
      </h5>
    ...
    <form action="{{route('grados.update',$grado->id)}}" method="POST" id="form_grado" name="form_grado">
        ...
        <div class="col-sm-9">
          <select id="escuela_id" name="escuela_id" class="form-control" required>
            @foreach($escuelas as $escuela)
              ...
              @endif
              <option value="{{ $escuela->id }}" {{ $escuela->id === $grado->escuela_id ? "selected" : "" }}>{{ $escuela->nombre }} ({{$escuela->nivel->nombre}})</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-sm-6">
          <input type="text" class="form-control" id="nombre" name="nombre" placeholder="Nombre del grado" value="{{ $grado->nombre }}" required>
        </div>
      </div>
      <div class="form-group row">
        <label for="abreviacion" class="col-sm-3 col-form-label">Abrev.</label>
        <div class="col-sm-6">
          <input type="text" class="form-control" id="abreviacion" name="abreviacion" placeholder="Abreviacion del grado" value="{{ $grado->abreviacion }}">
        </div>
      </div>
